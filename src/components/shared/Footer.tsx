...
const COOL_FACTS: CoolFact[] = [
  ...
  },
  {
    ...
    titleEn: "Malus's Law",
    titleZh: '马吕斯定律',
    contentEn: '$I = I_0\\cos^2\\theta$ — the fundamental law of polarization',
    contentZh: "$I = I_0 \\cos^2 \\theta$ — 偏振光学的基本定律",
    scientistEn: 'Etienne-Louis Malus',
    scientistZh: '马吕斯',
  },
  {
  ...
  },
  {
    ...
    titleZh: '彩虹的秘密',
    contentEn: 'Light from rainbows is partially polarized, especially at $42^{\\circ}$ from the sun',
    contentZh: "彩虹的光是部分偏振的，尤其在与太阳成$42^{\\circ}$角时",
  },
  ...
  },
  {
    ...
    titleZh: '屏幕偏振测试',
    contentEn: 'Rotate polarized sunglasses in front of your phone screen — watch it go dark at $90^{\\circ}$!',
    contentZh: "在手机屏幕前旋转偏振太阳镜——旋转$90^{\\circ}$时屏幕会变黑！",
  },
  {
    type: 'experiment',
...
// 自定义hook：获取两个随机的cool facts
function generateRandomFacts(): [CoolFact, CoolFact] {
  // 随机选择两个不同的类型
  const shuffledTypes = [...FACT_CATEGORIES].sort(() => 0.5 - Math.random())
...
// 根据时间间隔刷新facts的hook - 返回两个不同类型的facts,每5秒（5000ms)刷新一次
function useCoolFacts(refreshInterval = 5000): [CoolFact, CoolFact] {
  const [facts, setFacts] = useState<[CoolFact, CoolFact]>(generateRandomFacts())

  useEffect(() => {
    // 设置定时器定期刷新facts
    const interval = setInterval(() => {
      setFacts(generateRandomFacts())
    }, refreshInterval)
    // 清理定时器
...
const ABOUT_LINKS: FooterLink[] = [
  { labelKey: 'GitHub', labelZh: 'GitHub', path: 'https://github.com/amatke31/polarcraft', external: true, icon: 'github' },
]
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        >
          {MathText({ text: isZh ? fact.contentZh : fact.contentEn })}
        </p>
        {fact.scientistEn && (
          <p
            className={cn(
